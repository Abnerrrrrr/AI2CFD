...
from PINN.toy_1d.Pinn import PINN_1d_neuman
...
# 在此循环里进行RAR-G采样，假设训练100次，每次多采100个点
for i in range(1,101):
    ...
    model.train(n_iter=1000, optimizer_type='adam')
    ...
    residual = np.square(u_star-u_pred)
    X_f_train = selectSample(X_f_train,X_star,u_star,u_pred,100)
    # 每循环10次绘制一张残差的热力图像
    if i % 10 == 0:
        picture(residual)
# 绘制自适应采样之后的散点图
plot_sample(X_f_train)
###############################################################
# 在此循环里进行RAD采样，假设训练100次，每次多采100个点
for i in range(1,11):
    # 训练模型
    model2 = PINN_1d_neuman(X_0_train, X_1_train, X_t0_train, u_t0_train, X_f_train, layers, lb, ub, nu)
    ...
    residual = np.square(u_star-u_pred)
    # 计算概率密度函数
    density = p(residual, 1,1)    # 归一化概率密度函数
    X_f_train= adaptive_sampling(density,100,X_star)
    if i % 10 == 0:
        picture(residual)
# 绘制自适应采样之后的散点图
plot_sample(X_f_train)
